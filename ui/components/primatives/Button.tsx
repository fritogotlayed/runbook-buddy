...
export function Button(props: ButtonProps) {
  const { children, enableSpinner, class: propsClass, ...rest } = props;
  ...
  return (
    <button
      class={`px-3 py-1 ${borderClasses} rounded bg-white hover:bg-gray-100 transition-colors${
        propsClass ? ` ${propsClass}` : ''
      }`}
      {...rest}
    >
      {enableSpinner ? loadingSpinner : children}
    </button>
  );
}
