...
function mapV1UIInstanceItemToInstanceItem(
...
function mapV1UIInstanceItemsToInstanceItems(
...
const InstancesPage: NextPage = () => {
  ...
  const navigateToCreateWizard = () => {
    void router.push('instances/create');
  };
  ...
  const viewItemSaved = async () => {
    if (viewInstanceData) {
      ...
      const updateOriginalState = (
        ...
        const children = item.children.map((e: V1UIInstanceItem) =>
          updateOriginalState(e),
        );
        return {
          ...item,
    ...
    if (viewInstanceData) {
      ...
      const scanToReplace = (elem: V1UIInstanceItem): V1UIInstanceItem => {
        ...
        if (elem.children.length > 0) {
          elem.children = elem.children.map((e: V1UIInstanceItem) =>
            scanToReplace(e),
          );
          elem.childrenComplete = elem.children.reduce(
  ...
  const sidePanel = viewInstanceData ? (
    <ViewInstance
    ...
    <>
      <Grid item xs={xsWidth} md={mdWidth} lg={lgWidth}>
        <ListInstances
          ...
          onItemDeletedClick={(key) => void itemDeleted(key)}
          onItemViewClick={(key) => void viewItem(key)}
