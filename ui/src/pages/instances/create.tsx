...
import { convertTemplateToInstance } from 'utils/converters';
import {
  V1InstanceFile,
  V1InstanceItem,
  V1TemplateFile,
...
const InstanceCreatePage: NextPage = () => {
  ...
  const templateFieldUpdated = (key: string, value: string) => {
    ...
    const processInstanceItem = (
      ...
      item.data = item.data.replace(
        ...
        replacementMapping.get(replaceKey) || expression,
      );
      item.children.forEach((e) => processInstanceItem(e, replaceKey));
    };

    if (templateData && replaceKeys) {
      const workingData = convertTemplateToInstance(templateData);
      for (let i = 0; i < replaceKeys.length; i += 1) {
        // TODO: Validate these forEach's
        workingData.contents.forEach((e) =>
          processInstanceItem(e, replaceKeys[i]),
        );
      }
      setInstanceData(workingData);
    }
  ...
  if (templateId === undefined) {
    leftPanel = <SelectTemplate onTemplateSelected={templateSelected} />;
  } else {
    leftPanel = (
      <CreateInstance
        keys={replaceKeys}
        onFieldUpdated={templateFieldUpdated}
        onCreateButtonClick={createButtonClicked}
      />
    );
    rightPanel = <ViewTemplate templateId={templateId} data={instanceData} />;
  }
  ...
  return (
    <>
      ...
      <Grid
        ...
        md={6}
        lg={6}
        display={rightPanel ? undefined : 'none'}
      >
        {rightPanel}
      </Grid>
    </>
