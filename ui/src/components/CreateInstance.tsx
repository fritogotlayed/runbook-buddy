...
export type CreateCallback = (name: string) => Promise<void> | void;
export type FieldUpdatedCallback = (
  key: string,
  value: string,
) => Promise<void> | void;
...
export default function CreateInstance(props: ICreateInstanceProps) {
  ...
  const updateField = (key: string, value: string) => {
    if (onFieldUpdated) {
      void onFieldUpdated(key, value);
    }
  };

  const createClicked = () => {
    if (name && onCreateButtonClick) {
      void onCreateButtonClick(name);
    }
  };
  ...
  if (keys && keys.length > 0) {
    ...
    inputFields = <div>No fields to replace!</div>;
  }
