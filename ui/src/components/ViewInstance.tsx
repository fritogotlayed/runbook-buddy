...
import { DeviceHubTwoTone, Save } from "@mui/icons-material";
...
export type ItemToggledCallback = (item: V1UIInstanceItem) => void;
...
export default function ViewInstance(props: IViewInstanceProps) {
  ...
  const toggleItemCompleted = (key: string) => {
    if (onItemUpdated) {
      const recursiveFind = (items: V1UIInstanceItem[]): V1UIInstanceItem | undefined => {
  ...
  const isItemOrChildrenChanged = (item: V1UIInstanceItem): boolean => {
    ...
    return item.children.map((c: V1UIInstanceItem) => isItemOrChildrenChanged(c)).reduce((prev, curr) => prev || curr, false);
  }

  // const isDirty = data.length === 0 ? false : data.map((e) => e.completed !== e.originalState).reduce((prev, curr) => prev || curr);
  const isDirty = data.length === 0 ? false : data.map((e) => isItemOrChildrenChanged(e)).reduce((prev, curr) => prev || curr, false);
  ...
  if (data) {
    ...
    const processForUIElement = (list: JSX.Element[], item: V1UIInstanceItem): void => {
      if (!item.visible) return;

      const depth = getDepth(item);
      const marginLeft = `${depth * 1}em`;
      const nestedChildCount = (me: V1UIInstanceItem): number => {
        const childCount = me.children.reduce((prev, curr: V1UIInstanceItem) => prev + nestedChildCount(curr), 0)
        return me.children.length + childCount;
      };
      const labelSuffix = item.children.length === 0 ? '' : ` ${item.childrenComplete} / ${nestedChildCount(item)}`;
      ...
      item.children.forEach(e => processForUIElement(list, e as V1UIInstanceItem));
    }
