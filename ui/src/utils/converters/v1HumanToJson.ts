...
function getProperParent(
  previous: V1TemplateTreeItem,
  ...
  return getProperParent(
    previous.parent as V1TemplateTreeItem,
    depthAdjustment - 1,
  );
}

function buildTemplateTreeItems(
  lineData: string,
  nextLines: string[],
  previous: V1TemplateTreeItem,
  parentDepth: number,
...
function convertToV1TemplateItemList(
  data: V1TemplateTreeItem[],
): V1TemplateItem[] {
  return data.map((e) => {
...
export function convertTemplateToJSON(input: string): string {
  ...
  if (lines && lines.length > 0) {
    ...
    buildTemplateTreeItems(
      firstData,
      lines.slice(1),
      { data: 'N/A', children: workingData },
      -1,
    );
