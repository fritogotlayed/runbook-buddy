...
function getFilesWithExtensions(data: string[], extensions: string[]): string[] {
  return data.filter((val) => extensions.map((ext) => val.endsWith(ext)).reduce((prev, curr) => prev || curr));
}
...
async function upgradeDataFilesOnce(log: FastifyLoggerInstance): Promise<boolean> {
  // Get files in store
  ...
  for (let i = 0; i < templateFiles.length; i++) {
    ...
    if (converter) {
      ...
      await writeFileContents(result.updatedDestinationFileName, result.destinationFileBody, false);
      performedUpgrade = true;
      log.info({ fileName, fromVersion: result.fromVersion, toVersion: result.toVersion }, 'Data file updated.');
    }
  }
...
export async function updateDataFiles(log: FastifyLoggerInstance): Promise<void> {
